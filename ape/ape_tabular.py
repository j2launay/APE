...
class ApeTabularExplainer(object):
    ...
    def __init__(self, train_data, class_names, black_box_predict, black_box_predict_proba=None,
                multiclass=False, continuous_features=None, categorical_features=[],
                categorical_values=[], feature_names=None, discretizer="quartile", 
                nb_min_instance_in_field=800, threshold_precision=0.95, 
                nb_min_instance_per_class_in_field=100, verbose=False, 
                categorical_names=[], linear_separability_index=0.99,
        ...
        if continuous_features is None:
            self.continuous_features = [x for x in range(len(train_data[0])) if x not in categorical_features]
        else:
            self.continuous_features = continuous_feature
        ...
        self.anchor_explainer = anchor_tabular.AnchorTabularExplainer(class_names, feature_names, self.train_data, 
                                                                    ...
                                                                    black_box_labels=self.black_box_labels, 
                                                                    ordinal_features=self.continuous_features)
    ...
    def explain_instance(self, instance, opponent_class=None, growing_method='GF', n_instance_per_layer=2000, first_radius=0.01, 
                        nb_features_employed=None, dicrease_radius=10, linear_model=None, all_explanations_model=False,
                        distance_metric="w_euclidian"):
        """
        ...
        if self.multimodal_results:
            ...
            ape_accuracy, ape_coverage, ape_f2, ape_explanation = self.compute_anchor_accuracy_coverage(instance, 
                                        ...
                                        nb_instance_test_data_label_as_target,
                                        growing_method=growing_method)
            return ape_explanation, ape_coverage, ape_accuracy, self.clusters_centers, "Multimodal"
        else:
            ...
            return ape_explanation, ape_coverage, ape_accuracy, self.closest_counterfactual, "Unimodal"
